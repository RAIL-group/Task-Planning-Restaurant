...
def test_ma_model_output_tall():
    use_cuda = torch.cuda.is_available()
    device = torch.device("cuda" if use_cuda else "cpu")
    net_name = 'ap_tall.pt'
    eval_net = taskplan_multi.models.gcn.AnticipateGCN.get_net_eval_fn(
        network_file='/data/restaurant-multi-tall/logs/beta-v0/' + net_name,
        device=device
    )
    root = "/data/restaurant-multi-tall/"
    json_files = list()
    ...
    for file in json_files:
        ...
        for idx, pickle_path in enumerate(df[0]):
            pickle_path = "/data/restaurant-multi-tall/"+pickle_path
