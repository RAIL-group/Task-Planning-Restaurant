...
def get_domain(types_dict=types_dict):
    ...
    (:action fix
        :parameters (?r - robot ?i - item ?t - item)
        :precondition (and
            ;(type ?l fixingstation)
            (type ?t toolbox)
            (type ?i tire)
            (rob-at ?r tirefixingstation)
            (is-at ?i tirefixingstation)
            (is-at ?t tirefixingstation)
            (is-bad ?i)
        )
        :effect (and
            (not (is-bad ?i))
    ...
    (:action repair
        ...
        :precondition (and
            ...
            (type ?t toolbox)
            (type ?i mirror)
            (rob-at ?r mirrorfixingstation)
            (is-at ?i mirrorfixingstation)
            (is-at ?t mirrorfixingstation)
            (is-bad ?i)
        )
        :effect (and
            (not (is-bad ?i))
    ...
    (:action ask-help
        ...
        :effect (and
            (robot-active ?r1)
            (increase (total-cost) 100) ; Cost of asking help
        )
    )
    )
    """
