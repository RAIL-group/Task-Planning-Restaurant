...
def load_restaurant(seed, agents):
    ...
    """
    room1_containers = ['tirerack','tirefixingstation']
    # random.shuffle(kitchen_containers_list)
    if BOT_1 in agents:
        room1_containers.extend([BOT_1])
    room2_containers = ['mirrorrack', 'mirrorfixingstation']
    if BOT_2 in agents:
        room2_containers.extend([BOT_2])
    if random.random() > 0.5:
        room2_containers.extend(['toolrack'])
    else:
        room1_containers.extend(['toolrack'])
    ...
    datum = generate_restaurant(seed, room1_containers,
                                room2_containers)
    return datum
...
class WORKSHOP:
    def __init__(self, seed, agents=[BOT_1, BOT_2], active=BOT_1):
        self.seed = seed
        self.agent_list = agents
        self.restaurant = load_restaurant(self.seed, agents)
        self.rooms = self.restaurant[ROOMS_KEY]
        self.doors = self.restaurant[DOORS_KEY]
        self.containers = self.restaurant[OBJECTS_KEY]
        ...
        for agent in agents:
            ...
            if self.restaurant[agent]['loc'] == ROOM_1:
                mother_poly = Polygon([(point['x'], point['z'])
                                            for point in self.rooms[ROOM_1]['polygon']])
            else:
                mother_poly = Polygon([(point['x'], point['z'])
                                            for point in self.rooms[ROOM_2]['polygon']])
        ...
        for container in self.containers:
            ...
            inflation_distance = INFLATE_UB
            relative_loc[container[ASSET_ID]] = point_cloud
    ...
    def set_occupancy_grid(self):
        ...
        kitchen = self.rooms['tirestation']['polygon']
        serving_room = self.rooms['mirrorstation']['polygon']
    ...
    def get_final_state_from_plan(self, plan):
        ...
        for obj_dict in objects:
            if obj_dict['assetId'] in fixed_objs:
                obj_dict['bad'] = 0
    ...
    def place_car_items(self, obj, loc, bad=0):
        state = self.get_current_object_state()
        new_objs = list()
        for objct in state:
            if objct['assetId'] == obj['assetId']:
                objct.update({'position': loc})
                objct.update({'bad': bad})
            new_objs.append(objct)
        return new_objs